...
class RepOrderListView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        orders = Order.objects.all().prefetch_related('items__product', 'user').order_by('-created_at')
        
        if status_filter:
            orders = orders.filter(status=status_filter)
        if branch_filter:
            orders = orders.filter(branch__iexact=branch_filter)
        if search:
...
class RepOrderUpdateView(APIView):
    permission_classes = [IsAuthenticated]
